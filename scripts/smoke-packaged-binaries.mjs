...
function shouldPreferSystemFfprobe() {
  return process.platform === "darwin" && process.arch === "arm64";
}

function assertPackagedFileMatchesSource(packagedPath, sourcePath, label) {
  if (!existsSync(sourcePath) || !statSync(sourcePath).isFile()) {
    return;
  }

  const packagedHash = hashFile(packagedPath);
  const sourceHash = hashFile(sourcePath);
  if (packagedHash !== sourceHash) {
    fail(
      `${label} hash mismatch: packaged ${relativePath(packagedPath)}=${packagedHash}, source ${relativePath(sourcePath)}=${sourceHash}`,
    );
  }
}
...
function getExpectedNativeHelperFiles(archTag) {
  if (archTag.startsWith("win32-")) {
    const helpers = [
      {
        name: "wgc-capture.exe",
        label: "Windows capture helper",
        executable: true,
      },
      {
  ...
  if (archTag.startsWith("darwin-")) {
    return [
      ...
      {
        name: "recordly-window-list",
        label: "Window list helper",
        executable: true,
      },
      {
        name: "recordly-system-cursors",
        label: "System cursor helper",
        executable: true,
      },
      {
...
function verifyFfmpeg(unpackedRoot) {
  ...
  if (!output.startsWith("ffmpeg version")) {
    fail(
      `FFmpeg version smoke returned unexpected output from ${relativePath(ffmpegPath)}`,
    );
  }
...
function verifyFfprobe(unpackedRoot) {
  ...
  const systemFfprobePath = shouldPreferSystemFfprobe()
    ? findSystemFfprobe()
    : null;
  const ffprobePath = systemFfprobePath ?? packagedFfprobePath;

  assertFile(
    ffprobePath,
    systemFfprobePath ? "system FFprobe binary" : "packaged FFprobe binary",
    {
      executable: true,
    },
  );
  ...
  if (!output.startsWith("ffprobe version")) {
    fail(
      `FFprobe version smoke returned unexpected output from ${relativePath(ffprobePath)}`,
    );
  }
...
function verifyNativeHelpers(unpackedRoot) {
  ...
  if (!existsSync(nativeBinRoot)) {
    fail(
      `native helper bin directory is missing at ${relativePath(nativeBinRoot)}`,
    );
  }
  ...
  for (const archTag of getRequiredArchTags()) {
    ...
    if (!existsSync(archDir)) {
      fail(
        `native helper arch directory is missing at ${relativePath(archDir)}`,
      );
    }
    ...
    for (const expectedFile of expectedFiles) {
      const packagedPath = path.join(archDir, expectedFile.name);
      const label = `${expectedFile.label} (${archTag})`;
      assertFile(packagedPath, label, {
        executable: expectedFile.executable,
      });
      assertPackagedFileMatchesSource(
        packagedPath,
        path.join(
          projectRoot,
          "electron",
