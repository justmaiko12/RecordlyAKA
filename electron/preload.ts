...
type NativeWebcamPreviewStopOptions = {
  reason?: "renderer-stop" | "recording-start";
};
...
contextBridge.exposeInMainWorld("electronAPI", {
  ...
  webcamLayoutToggle: (payload: {
    timeMs: number;
    mode: "screen" | "camera-full";
  }) => {
  ...
  getSelectedWebcamDevice: () => {
    return ipcRenderer.invoke("get-selected-webcam-device") as Promise<
      string | null
    >;
  },
  getVideoDeviceConnectionInfo: () => {
    return ipcRenderer.invoke("get-video-device-connection-info") as Promise<{
      success: boolean;
      devices: Array<{
        label: string;
  ...
  getAudioDeviceConnectionInfo: () => {
    return ipcRenderer.invoke("get-audio-device-connection-info") as Promise<{
      success: boolean;
      devices: Array<{
        label: string;
        normalizedLabel: string;
  ...
  nativeStaticLayoutExportCancel: (sessionId: string) => {
    return ipcRenderer.invoke(
      "native-static-layout-export-cancel",
      sessionId,
    ) as Promise<{
      success: boolean;
    }>;
  ...
  onNativeStaticLayoutExportProgress: (
    ...
    const listener = (
      _event: Electron.IpcRendererEvent,
      payload: NativeStaticLayoutProgress,
    ) => callback(payload);
    ipcRenderer.on("native-static-layout-export-progress", listener);
    return () =>
      ipcRenderer.removeListener(
        "native-static-layout-export-progress",
        listener,
      );
  },
  ...
  auditFinalizedRecording: (videoPath: string) => {
    return ipcRenderer.invoke("audit-finalized-recording", videoPath);
  },
  ...
  onRecordingDegraded: (
    ...
    ipcRenderer.on("recording-degraded", listener);
    return () => ipcRenderer.removeListener("recording-degraded", listener);
  },
  onNativeWebcamPreview: (
    callback: (state: NativeWebcamPreviewState) => void,
  ) => {
    const listener = (
      _event: Electron.IpcRendererEvent,
      payload: NativeWebcamPreviewState,
    ) => callback(payload);
    ipcRenderer.on("native-webcam-preview", listener);
    return () => ipcRenderer.removeListener("native-webcam-preview", listener);
  },
  startNativeWebcamPreview: (options?: NativeWebcamPreviewStartOptions) => {
    return ipcRenderer.invoke(
      "start-native-webcam-preview",
      options ?? {},
    ) as Promise<NativeWebcamPreviewStartResult>;
  },
  stopNativeWebcamPreview: (options?: NativeWebcamPreviewStopOptions) => {
    return ipcRenderer.invoke(
  ...
  onRecordingSessionChanged: (
    callback: (session: RecordingSessionData | null) => void,
  ) => {
    const listener = (
      _event: Electron.IpcRendererEvent,
    ...
    return () =>
      ipcRenderer.removeListener("recording-session-changed", listener);
  },
  ...
  startWebcamSidecarRecording: (options: {
    sessionId: string;
    fileName: string;
    mimeType?: string | null;
  }) => {
    return ipcRenderer.invoke("start-webcam-sidecar-recording", options);
  },
  appendWebcamSidecarChunk: (
    streamId: string,
    chunk: ArrayBuffer,
  ...
  finishWebcamSidecarRecording: (streamId: string) => {
    return ipcRenderer.invoke("finish-webcam-sidecar-recording", streamId);
  },
  abortWebcamSidecarRecording: (streamId: string, reason?: string | null) => {
    return ipcRenderer.invoke(
  ...
  recordRecordingEvent: (input: {
    sessionId: string;
    event: string;
    details?: Record<string, unknown>;
  }) => {
    return ipcRenderer.invoke("record-recording-event", input);
  },
  ...
  getRecordingAudioLabConfig: () =>
    ipcRenderer.invoke("get-recording-audio-lab-config"),
  setRecordingPreferences: (prefs: {
    microphoneEnabled?: boolean;
    microphoneDeviceId?: string;
    systemAudioEnabled?: boolean;
    webcamFrameRate?: number;
    webcamQualityMode?: string;
