...
type EditorCloseBehavior = {
  returnToLauncher: boolean;
  clearActiveSession: boolean;
};

const USER_EDITOR_CLOSE_BEHAVIOR: EditorCloseBehavior = {
  returnToLauncher: true,
  clearActiveSession: true,
};

const INTERNAL_EDITOR_REPLACE_CLOSE_BEHAVIOR: EditorCloseBehavior = {
  returnToLauncher: false,
  clearActiveSession: false,
};

const editorCloseBehaviors = new WeakMap<BrowserWindow, EditorCloseBehavior>();

function getEditorCloseBehavior(window: BrowserWindow): EditorCloseBehavior {
  return editorCloseBehaviors.get(window) ?? USER_EDITOR_CLOSE_BEHAVIOR;
}

function closeEditorWindowBypassingUnsavedPrompt(
  window: BrowserWindow | null,
  behavior: EditorCloseBehavior = USER_EDITOR_CLOSE_BEHAVIOR,
) {
  ...
  if (isEditorWindow(window)) {
    ...
    editorCloseBehaviors.set(window, behavior);
  }
  window.close();
}
...
function createEditorWindowWrapper() {
  ...
  if (previousWindow && !previousWindow.isDestroyed()) {
    ...
    if (closingEditorWindow) {
      closeEditorWindowBypassingUnsavedPrompt(
        previousWindow,
        INTERNAL_EDITOR_REPLACE_CLOSE_BEHAVIOR,
      );
    } else {
  ...
  editorWindow.on("closed", () => {
    const closeBehavior = getEditorCloseBehavior(editorWindow);
    editorCloseBehaviors.delete(editorWindow);
    ...
    if (closeBehavior.clearActiveSession) {
      clearActiveEditorSession();
    }
    if (
      closeBehavior.returnToLauncher &&
      !isQuitRequested &&
      !IS_SMOKE_EXPORT
    ) {
      setTimeout(() => {
    ...
    (recording: boolean, sourceName: string) => {
      ...
      if (recording) {
        ...
        registerSceneStyleShortcuts((mode) => {
          getHudOverlayWindow()?.webContents.send("scene-style-hotkey", {
            mode,
          });
        });
      }
  ...
  session.defaultSession.setDisplayMediaRequestHandler(
    async (_request, callback) => {
      try {
        ...
        if (!shouldUseElectronSourceEnumeration()) {
          callback({});
          return;
        }
