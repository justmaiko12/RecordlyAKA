...
type FfprobeBinaryPathDeps = {
  loadFfprobeStatic: typeof loadFfprobeStatic;
  resolveSystemFfprobeBinaryPath: typeof resolveSystemFfprobeBinaryPath;
...
function shouldPreferSystemFfprobe(platform: NodeJS.Platform, arch: string) {
  return platform === "darwin" && arch === "arm64";
}

export function resolveFfprobeBinaryPath(
  deps: Partial<FfprobeBinaryPathDeps> = {},
): string {
  const activeDeps: FfprobeBinaryPathDeps = {
    loadFfprobeStatic: deps.loadFfprobeStatic ?? loadFfprobeStatic,
    resolveSystemFfprobeBinaryPath:
      deps.resolveSystemFfprobeBinaryPath ?? resolveSystemFfprobeBinaryPath,
    existsSync: deps.existsSync ?? existsSync,
    isPackaged: deps.isPackaged ?? app?.isPackaged ?? false,
    platform: deps.platform ?? process.platform,
    arch: deps.arch ?? process.arch,
  };

  if (shouldPreferSystemFfprobe(activeDeps.platform, activeDeps.arch)) {
    const systemFfprobe = activeDeps.resolveSystemFfprobeBinaryPath();
    if (systemFfprobe) {
      return systemFfprobe;
    }
  }

  const ffprobeStatic = activeDeps.loadFfprobeStatic();
  if (ffprobeStatic && typeof ffprobeStatic === "string") {
    const bundledPath = activeDeps.isPackaged
      ? ffprobeStatic.replace(/\.asar([/\\])/, ".asar.unpacked$1")
      : ffprobeStatic;

    if (activeDeps.existsSync(bundledPath)) {
      return bundledPath;
    }
  }
...
export function getFfprobeBinaryPath(): string {
  return resolveFfprobeBinaryPath();
}
